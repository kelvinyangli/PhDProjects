...
cond_probs_adaptive = function(data, arities, sampleSize, targetIndex, curIndex, curPaIndices) {
  ...
  for (i in 1:(sampleSize - 1)) {# counting adaptively at each data point
    ...
    indices = as.numeric(data[i, c(curPaIndices, curIndex)])
    cnt[[matrix(indices, 1)]] = cnt[[matrix(indices, 1)]] + 1 # updating cnt
    indices = as.numeric(data[i + 1, c(curPaIndices, curIndex)]) # computing prob from 2nd data point
    for (k in 1:arities[targetIndex]) {# loop through each value of the target 
      
      indices[which(c(curPaIndices, curIndex) == targetIndex)] = k
