...
################################################## msg len ############################################
msgLen = function(data, y, x, sigma = 3, fullLen = FALSE) {
  ...
  if (fullLen) {
    partialLen = 0.5*nFreePar*log(2*pi) + nFreePar*log(sigma) - 0.5*log(arityDependVar) - 
      0.5*sum((arityIndependVar - 1)*log(arityDependVar) + (arityDependVar - 1)*log(arityIndependVar)) + 0.5*nFreePar*(1 + log(latticeConst))
  }
  mml =  partialLen + (1/(2*sigma^2))*sum(negLogLike$par^2) + 0.5*log(fisher) + negLogLike$value 
...
mbMML = function(data, y, debug = FALSE, fullLen = FALSE, nNodeInMB = FALSE) {
  allNodes = names(data)
  ...
  repeat{
    ...
    for (i in 1:length(unUsedNodes)) {
      res[i] = msgLen(data = data, y = y, x = c(unUsedNodes[i], cmb), fullLen = fullLen)$mml
