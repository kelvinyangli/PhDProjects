...
dataPreprocess = function(data, y, x) {
...
dataPreprocess = cmpfun(dataPreprocess)
...
#
######################################################################################################################
fisherMatrixBinary = function(dependVar, independVar, beta, nFreePar = nFreePar) {
  ...
  FIM[1, 1] = secondDeri(beta, 1, 1)
  for (j in 2:nrow(FIM)) {
    for (k in 2:j) FIM[j, k] = secondDeri(beta, j, k)
  }
  FIM[,1] = diag(FIM)
...
fisherMatrixBinary = cmpfun(fisherMatrixBinary)

fisherMatrix = function(dependVar, independVar, beta, nFreePar = nFreePar) {
  # 2nd derivative of nll
  secondDeri = function(beta, j, k) {
    LL = 0 
    for (i in 1:length(dependVar)) {
      expBetaDotX = exp(sum(beta*independVar[i,]))
  ...
  FIM = matrix(NA, nFreePar, nFreePar) 
  for (j in 1:nrow(FIM)) {
    for (k in 1:j) FIM[j, k] = secondDeri(beta, j, k)
  }
  FIM[upper.tri(FIM)] = t(FIM)[upper.tri(t(FIM))]
  return(FIM)
}
...
################################################## msg len ############################################
msgLen = function(data, y, x, sigma = 3, fullLen = FALSE, binary = TRUE) {
  tempData = dataPreprocess(data = data, y = y, x = x)
  ...
  index = c()
  for (i in 1:length(arityIndependVar)) index = c(index, sum(arityIndependVar[1:i]) + 1)
  
  if(binary) {
    fisherInfoMatrix = fisherMatrixBinary(dependVar = dependVar, independVar = independFreeVar, 
                             beta = negLogLike$par[-index], nFreePar = nFreePar)
  } else {
    fisherInfoMatrix = fisherMatrix(dependVar = dependVar, independVar = independFreeVar, 
                       beta = negLogLike$par[-index], nFreePar = nFreePar)
  }
  ...
  partialLen = 0
  if (fullLen) {
    partialLen = 0.5*nFreePar*log(2*pi) + nFreePar*log(sigma) - 0.5*log(arityDependVar) - 
      0.5*sum((arityIndependVar - 1)*log(arityDependVar) + 
                (arityDependVar - 1)*log(arityIndependVar)) + 0.5*nFreePar*(1 + log(latticeConst))
  }
  mml =  partialLen + (1/(2*sigma^2))*sum(negLogLike$par^2) + 0.5*logFisher + negLogLike$value 
  lst = list(negLogLike$par, negLogLike$value, logFisher, mml)
...
mbMML = function(data, y, debug = FALSE, sigma = 3, fullLen = FALSE, nNodeInMB = FALSE, binary = TRUE) {
  allNodes = names(data)
  ...
  repeat{
    ...
    for (i in 1:length(unUsedNodes)) {
      result = msgLen(data = data, y = y, x = c(unUsedNodes[i], cmb), sigma = sigma, fullLen = fullLen, binary = binary)
      res[i] = result$mml
      #res[i] = msgLen(data = data, y = y, x = c(unUsedNodes[i], cmb), sigma = sigma, fullLen = fullLen, binary = binary)
      
      if (debug) {
        cat("    >", unUsedNodes[i], "has msg len:", round(res[i], 2), paste0("(", round(result$logFisher, 2), ")"), "\n")  
      } # then
    }
