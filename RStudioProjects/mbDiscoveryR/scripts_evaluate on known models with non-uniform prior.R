...
# generate data 
models = list.files(paste0(model, " nonUniform/cpts/"))
for (i in 1:length(models)) {
  
  cpts = readRDS(paste0(model, " nonUniform/cpts/", models[i]))
  ...
  for (j in 1:nIter) { 
    ...
    write.table(data, paste0(model, " nonUniform/data/", fileName, ".data"), row.names = FALSE, col.names = FALSE) # save data
...
# apply mmlCPT 
datasets = list.files(paste0(model, " nonUniform/data/"), pattern = paste0("_", n, "_"))
for (ii in 1:length(datasets)) {
  
  data = read.table(paste0(model, " nonUniform/data/", datasets[ii]))
  ...
  saveRDS(mbList, paste0(model, " nonUniform/mb/cpt std/", datasets[ii], ".rds")) # save mbList into folder
  
  # use symmetry condition to re-check for mb candidate for each node
  mbList = symmetryCheck(allNodes, mbList)
  
  saveRDS(mbList, paste0(model, " nonUniform/mb/cpt sym/", datasets[ii], ".rds")) # save mbList into folder
  
} # end for ii

computeStats3(model, "cpt std", n, nIter = nIter, alpha = 0.05, nDigits = 2)
computeStats3(model, "cpt sym", n, nIter = nIter, alpha = 0.05, nDigits = 2)
...
for (ii in 1:length(datasets)) {
  ...
  file.copy(paste0("../", model, " nonUniform/data/", datasets[ii]), paste0(model, ".data"), overwrite = TRUE) # copy data from "alarm data" to "pcmb" with new name "alarm.data"
  # notice that since data is copied to the same directory with the same name, they will be replaced by each other
  # also since we don't order datasets in "alarm data" folder, so the order of results is different from order of results in mmlcpt
  # but that's not a problem, since we only consider average, but not individual result
  ...
  saveRDS(mbList, paste0("../", model, " nonUniform/mb/iamb/", datasets[ii], ".rds")) # save learned mb as .rds
...
setwd("../")
computeStats3(model, "pcmb", n, nIter = 10, alpha = 0.05, nDigits = 2)
computeStats3(model, "iamb", n, nIter = 10, alpha = 0.05, nDigits = 2)
