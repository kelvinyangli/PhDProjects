...
sa = function(adjmtx, vars, dataInfo, n, step = 0.02, maxItr = 100, debug = FALSE) {
  
  for (nStep in 1:maxItr) {
    
    if (debug) cat("Step -", nStep, " - temperate", T, "\n")
    
    T <- (1 - step) ^ nStep # update temperate 
    ...
    if (length(nbr) > 0) {# if there are nbr, then sample 1 index from nbr
      ...
      if (acyclic(matrix2dag(adjmtx))) {# if contains no cycle 
        ...
        if ((next_mml >= current_mml) && runif(1, 0, 1) > exp(-(next_mml - current_mml) / T)) {
