...
```{r global_options, include=F, warning=F, message=F, eval=F}
knitr::opts_chunk$set(tidy.opts = list(width.cutoff = 60), tidy = TRUE, echo = TRUE, eval = T)
...
calculate_fim = function(prodPij1, prodPij2, px, probsMatrix, py1, py2, arities, yIndex) {
  # empty FIM
  ...
  for (rowIndex in 2:(nrow(fim) - 1)) {
    if (ncol(fim) - rowIndex == 1) {
      fim[rowIndex, -(1:rowIndex)] = 
        sum(-mm * py1 * py2 / (probsMatrix[, rowIndex - 1] * probsMatrix[, -(1:(rowIndex - 1))]))
    } else {
      fim[rowIndex, -(1:rowIndex)] = 
        colSums(-mm * py1 * py2 / (probsMatrix[, rowIndex - 1] * probsMatrix[, -(1:(rowIndex - 1))]))
    } # end else
  }
...
mml_NB = function(data, pars, fim, px, xIndices, yIndex, debug = FALSE) {
  ...
  nll = 
    -sum(apply(data, 1, nll_auxiliary, pars = pars, xIndices = xIndices, yIndex = yIndex)) + 
    sum(log(px))
...
## Random model
```{r}
dag = empty.graph(c("Y", paste0("X", 1:5)))
...
# FIM
fim = calculate_fim(prodPij1, prodPij2, px, probsMatrix, py1, py2, arities, yIndex)
# mml
...
```{r}
di = getDataInfo(data)
mbForwardSelection.fast(data, names(data)[yIndex], di$arities, di$indexListPerNodePerValue, debug=F)
(i_cpt = mmlCPT(yIndex, xIndices, di$indexListPerNodePerValue, di$arities, n))
```
...
debug = T
allNodes = names(cpts)
# mbForwardSelection(data, y, mmlLogit, arities, indexListPerNodePerValue, 
#                      base = exp(1), indicatorMatrix = indicatorMatrix, mbSize = 1000, 
#                      interaction = FALSE, debug)

(i_lr = msgLenWithPredictors(data, indicatorMatrix, yIndex, xIndices, arities, allNodes, sigma=3))
#mmlLogit(data, indicatorMatrix, yIndex, xIndices, arities, allNodes, sigma=3)
```
