...
## Exhaustive search
```{r}
# is_wrs8 = function(graph, debug = F) {

# a vector to store all actions have done so far
#actions = matrix(0, 0, 2)
# a list to store all options

optList = simList = simNbrList = bkupList = list()
nVisited = nRemain = c()
restoreTo = NULL
# 
#cachedEdges = matrix(0, 0, 2)
...
# repeat {
  ...
  if (is_chordal(graph)) {
    wrs = 1
    if (debug) cat("The remaining graph is chordal! \n")
    break
  }
  
  # skip
  if (is.null(restoreTo)) {
    simInG = find_simplicial(graph)
  } else {
    simInG = simList[[restoreTo]]
  }
  ...
  if (length(simInG) > 0) {
    ...
    if (is.null(restoreTo)) {
      ...
      # removing all sim nodes from graph simultaneously
      subG = subgraph(graph, nodes = simInG, type = "nodes") 
      
      # find sim nodes' nbrs in the original graph
      if (debug) cat("nbrs: \n")
      for (x in simInG) {
        xNbrs = find_nbr(graph, x)
        xNbrs = xNbrs[!xNbrs %in% simInG]
        simNbrList[[x]] = xNbrs
        if (debug) {
          #cat()
          cat(x, "-", xNbrs, "\n")
    ...
    # list all unique sim nodes' nbrs
    nbrs = unique(unlist(simNbrList[simInG]))
    
    # omit those that have been deleted as sim nodes
    nbrs = nbrs[!nbrs %in% simInG]
    
    if (debug) cat("** re-arrange: \n")
    # finding their nbrs restricted to simNbrsList
    if (is.null(restoreTo)) {
      nbrsNbrList = find_nbrsNbrList(simNbrList[simInG], nbrs, debug)
    } else {
      nbrsNbrList = find_nbrsNbrList(simNbrList[simInG], nbrs, debug = F)
      nbrsNbrList[[restoreTo]] = nbrsNbrList[[restoreTo]][!nbrsNbrList[[restoreTo]] %in% optList[[restoreTo]][[nVisited[restoreTo]]]]
      for (x in optList[[restoreTo]][[nVisited[restoreTo]]]) {
        nbrsNbrList[[x]] = nbrsNbrList[[x]][nbrsNbrList[[x]] != restoreTo]
      }
      if (debug) {
        for (x in nbrs) {
          cat(x, "-", nbrsNbrList[[x]], "\n")
    ...
    for (x in nbrs) {
      ...
      # if x is a sim node in subG then leave it
      if (is_simplicial(subG, x)) {
        if (debug) cat(x, "- 0 \n")
      } else {
        
        # remove all x's nbrs in a temp graph
        tempG = subG
        xPartialNbrs = nbrsNbrList[[x]]
        tempG[x, xPartialNbrs] = tempG[xPartialNbrs, x] = 0
        
        # if x has the potential to be a sim node...
        if (is_simplicial(tempG, x)) {
          
          # ... finding all options of edge cut
          xNbrs = names(which(subG[x, ] == 1))
          subset = xNbrs[!xNbrs %in% xPartialNbrs]
          opts = all_edges_cut(subG, x, xNbrs, subset) # all options 
          
          # ... if there are more than 1 choice then store them all 
          # and back up the current subgraph for future restore
          if (length(opts) > 1) {
            simList[[x]] = simInG
            optList[[x]] = opts
            bkupList[[x]] = subG
          ...
          # ... update visited and remaining counts
          if (!x %in% names(nVisited)) {
            nVisited[x] = 1
            nRemain[x] = length(opts) - 1
          } 
          
          # else {
          #   nVisited = nVisited[x] + 1
          # }
          # nRemain[x] = length(optList[[x]]) - nVisited[x]
          
          # ... remove edges according to the current option in optList 
          subG = subgraph(subG, from = x, to = optList[[x]][[nVisited[x]]], type = "edges")
          if (debug) {
            cat(x, "-", optList[[x]][[nVisited[x]]], "\n")
          }
  ...
  } else if (sum(nRemain) > 0) {# try other options
    
    # find the nearest restore point
    restoreTo = names(tail(nRemain[nRemain > 0], 1))
    
    # restore to the graph backuped at that stage before removing edges from optList
    subG = bkupList[[restoreTo]]
    
    # update counts
    nVisited[restoreTo] = nVisited[restoreTo] + 1
    nRemain[restoreTo] = nRemain[restoreTo] - 1
    
    # remove edges according to the current nVisited
    from = restoreTo
    to = optList[[restoreTo]][[nVisited[restoreTo]]]
    subG = subgraph(subG, from = from, to = to, type = "edges")
    
    if (debug) {
      cat("*** try alternative *** \n")
      cat("restore to:", from, "-", nVisited[from], "\n")
      cat("** remove edges:", from, "-", to, "\n")
    }
