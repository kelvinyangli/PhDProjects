...
for (i in 1:length(dts)) {
  mb_pcmb = system(paste0("./kmb4_linux ", n, "/", dts[i], " ", n, " 30 -1 1.0 0 1 0.01"), intern = TRUE) # call c++ pcmb
  mb_pcmb = pcmb2mtx(mb_pcmb, "OR")
  write.table(mb_pcmb, paste0("~/Documents/Experiments/mb_consistency_test_16_may_2019/mb_pcmb/", n, "/", dts[i]), row.names = F, col.names = F)
}

# test morality
n = 5000
mbs = list.files(paste0(dir, "mb_pcmb/", n))
moral = nedges = rep(0, length(mbs))
for (j in 1:length(mbs)) {
  mtx = read.table(paste0(dir, "mb_pcmb/", n, "/", mbs[j]))
  mtx = as.matrix(mtx)
  colnames(mtx) = c()
  nedges[j] = sum(mtx) / 2
  # test morality
...
pcmb2mtx = function(output, rule = c("AND", "OR")) {
  ...
  for (i in 1:nvars) {
    ...
    if (!is.na(ind)) {
      lst[[i]] = as.numeric(strsplit(ind, ",")[[1]]) + 1
    } else {
