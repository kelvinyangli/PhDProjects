...
for (i in 1:length(datasets)) {
  ...
  saveRDS(mbrand, paste0(dir, params, "/rand_nbr/rand_", filename))
...
for (x in true) {
  ...
  m[k, 2] = round(sum(sapply(mbt, length)) / length(mbt), 2)
  k = k + 1
  
}
m[, 3] = n 
l = 4
for (folder in c("cpt", "nb", "rand", "rand_dag", "rand_nbr", "iamb", "inter_iamb")) {
  
  learned = list.files(paste0(dir, model, "/", folder))
  
  vv = vsed = c() # a vector of sed
  for (i in 1:length(true)) {
    ...
    for (k in 1:length(mbt)) {
      sed = c(sed, mb_false_finding(mbt[[k]], mbl[[k]]))
      re = paste0(round(conf_int(sed), 2), collapse = "+-")
    }
    vv = c(vv, sed)
    vsed = c(vsed, re)
  ...
  m[, l] = vsed
  l = l + 1
  cat(mean(vv), "--", 1.96*sd(vv)/sqrt(length(vv)), "\n")
}
