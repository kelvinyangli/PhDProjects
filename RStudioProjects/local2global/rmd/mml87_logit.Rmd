...
for (j in 1:nrep) {
  ...
  for (i in 1:length(vars)) {
    accLogit[i, -3] = unlist(edit_dist_mb(bnlearn::mb(dag, vars[i]), lstLogit[[i]], vars, vars[i]))
    accCPT[i, -3] = unlist(edit_dist_mb(bnlearn::mb(dag, vars[i]), lstCPT[[i]], vars, vars[i]))
  }
