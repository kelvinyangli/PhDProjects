...
- Here is an example of a causal model, called the Alarm network. It is a medium-sized model with 37 variables, so learning back this generating model strcutre isn't too difficult with thousands of samples. 
- But imagine a large structure contains hundreds/thousands of variables. In this case, neither approximate nor exact causal learners can finish the task within a reasonable time while still keep similar accuracy as for smaller models. 
...
- It was developed as a way to balance the complexity of a statistical model and the fit of this model to a given dataset. 
...
- Since it is infeasible to calculate the strict MML in most cases, we use the MML87 approximations for the two parts. 
...
- In general, most of these terms aren't too difficult to calculate, except for the Fisher, which is the determinant of the FIM, and the FIM is ...

## Markov blanket discovery 
- The problem of learning the MB candidates of a varaible is call MB discovery. 
- There are two major approaches in MB disc, the first approach uses CI-test to assess whether or not a variable belongs to the MB. This approach normally starts with empty set and gradually include candidates along its way. 
...
- The definition of MB makes the smallest informative variable subset for a target. Therefore, we can use MML with an appropriate predictive model to search for such a subset. 
- The models we used are ...

## MB Disc using MML FOM
- The FOM is a simple model by omitting the higher order interaction terms. In other words, it assumes the input variables' predictive power for the target are additive. It is a strong assumption that is unlikely to be real but captures a certain degree of information for the target. The benefit of having this assumption is to reduce the sample size for predicting the target. 
- Most work for FOM has been done in the 1st year. 
- The message length of FOM can be calculated using this formula. 
- However, the calculation is inefficient due the complexity of the FIM, which has no closed form solution for its determinant. 
...
- Same as the FOM case, direct calculation of the message length for NB is inefficient and unstable due to the complex Fisher. The detail of its Fisher is written in the report. 
- Hence we use the adaptive code approach to calculate its MML score. 
...
- The MBMML algorithm was tested against the SLL algorithm on 11 real models. For each model, we tested the algorithms on 3 different sample sizes. The average performance of these learners over 10 datasets are reported with a 95 per cent confidence intervals. 
- The reported statistic is the edit dist, which is defined as the sum of false findings b/w the learned and true MBs. 
- SLL is also a LGL causal disc algorithm that uses the concept of MB. It is an exact algorithm that intend to find the true model structure by using dynamic programming and the BDe metric. 
- It is the 2nd MB disc approach that we mentioned early, i.e. by learning a sub-str around the target and reading off its MB. 
...
- Although we limited the local structures to be singly connected, polytrees provide sufficient connections for causal discovery. 
