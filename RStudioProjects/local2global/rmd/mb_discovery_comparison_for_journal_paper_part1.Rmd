...
for (i in 1:length(datasets)) {
  ...
  saveRDS(mbcpt, paste0(dir, model, "/mml_cpt/", filename, ".rds"))
...
for (ii in 1:length(datasets)) {
  ...
  #data = readRDS(paste0(dir, model, "/data_csv/", n, "/", datasets[ii]))
  ...
  saveRDS(mbiamb, paste0(dir, model, "/iamb/iamb_", filename))
...
```{r}
x = c(4.5,6.3,4.6,6.2,5.7,6.3,3.3,5.1,3.5,5.2,3.5,4.4,
      2.6,4.5,3,4.7,2.6,2.9)
x = c(6.4,8.4,6.5,8.2,7.5,8,5.1,6.9,5.2,7.2,5.5,6.2,
      4.2,6.1,4.5,6.5,4.4,4.4)
m = matrix(x, 3, 6, byrow = T)
m = cbind(c(500, 2000, 5000), m)
colnames(m) = c("nmb", "cpt", "nb", "random", "iamb", "pcmb", "sll")
m = data.frame(m)
m_melt = melt(m, id = "nmb")
colnames(m_melt)[2] = "alg"
error = c(0.3,0.3,0.3,0.3,0.3,0.3,0.2,0.3,0.2,0.3,0.2,0.3,0.2,0.2,0.2,0.3,0.2,0.2)
figure = ggplot(m_melt, aes(x = nmb, y = value, group = alg, colour = alg)) + 
  ylab(label = "edit dist") + xlab("sample size") + geom_line(aes(linetype = alg)) + geom_point(aes(shape = alg)) + 
  ylim(0, 10) + xlim(500, 5000) + guides(linetype = guide_legend()) +
  theme(legend.key.width = unit(1.5, "cm")) +
  geom_errorbar(aes(ymin = value - error, ymax = value + error), width = 0.03)
figure
filename = "ed_vs_samplesize_50_5_4_1"
ggsave(filename, device = "pdf", path = "~/Documents/Journal KDD/figures/")
