...
for (target in vars) {
  ...
  targetIndex = which(vars == target)
  targetAdptProbs = adpt_code_prob_no_pa(data, arities, targetIndex, n)
  lrand2[[i]] = f(data,di,arities,vars,target,n)
...
## using TOM prior in camml; also through in the true str score, it should win in most case, perhaps loss in small samples; testing random; sanity check, should be uniform, 100 smaples uniform is 100. 
```{r}
dag = randDag(7, 3)
#graphviz.plot(dag)
cpts = randCPTs(dag, 3, 1)
...
for (target in vars) {
  ...
  r2 = f2(dag, data,di,arities,vars,target,n, debug = T)
  cat(r1, "\n")
  cat(r2, "\n")
}
