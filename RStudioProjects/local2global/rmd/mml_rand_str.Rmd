...
## rand (with 100 secs samples)
## without sym: 22.8 +- 1.59
## with sym: 27.2 +- 2.42
```{r}
...
# model = "mml_rand"
# files = list.files(paste0("~/Documents/Experiments/mb_discovery/child/", model, "/", n, "/without_symmetry/"))
# for (i in 1:length(files)) {
#   mbrand = readRDS(paste0("~/Documents/Experiments/mb_discovery/child/", model, "/", n, "/without_symmetry/", files[i]))
#   for (j in 1:length(mbrand)) {
#     edrand[[i]][j] = mbEditDist(mbtrue[[j]], mbrand[[j]], vars[j], vars)
#   }
# }
# mean(sapply(edrand, sum))
...
for (target in vars) {
  ...
  forward_greedy(data, arities, vars, n, target, "nb", targetProbsAdpt = targetProbsAdpt, debug = T)
