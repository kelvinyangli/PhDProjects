...
## Testing on real models. Due to computation time, only tested on the 1st dataset for 500 samples. In terms of the sum of edit distance over all vars, cpt 24, nb 27, rand 20. 
```{r}
...
ed = rep(list(ed), length(datasets))
...
for (i in 1:length(datasets)) {
  ...
  for (j in 1:nvars) {
    ...
    # ed[[i]][j] = mbEditDist(mbtrue[[j]], mbrand[[j]], vars[j], vars)
    # mbcpt[[j]] = forward_greedy_fast(data, di, arities, n, vars[j], debug = F)
    # edcpt[j] = mbEditDist(mbtrue[[j]], mbcpt[[j]], vars[j], vars)
    mbnb[[j]] = forward_greedy(data, arities, vars, n, vars[j], "nb", debug = F)
    ednb[j] = mbEditDist(mbtrue[[j]], mbnb[[j]], vars[j], vars)
  }
