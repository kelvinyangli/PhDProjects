...
## Testing on real models. Due to computation time, only tested on the 1st dataset for 500 samples. In terms of the sum of edit distance over all vars, rand 20--22, cpt 24--16, nb 27--36. On 5000 samples, rand 12--4, cpt 15--10, nb 17--20.

## without sym: rand 24.5--2.11; cpt 25.5--1.02; nb 24.4--2.82; 
## with sym: rand 25.4--3.79; cpt 18--1.94; 31.2--4.61; find a way to drop the ed for rand after symmetry check!!!
```{r}
dir = "~/Documents/Experiments/UAI_exp/child/"
dag = readRDS(paste0(dir, "dag/child.rds"))
n = 500
...
for (i in 1:nvars) {
  mbtrue[[i]] = bnlearn::mb(dag, vars[i])
}

# edcpt = edrand = ednb = rep(0, nvars)
ed = rep(0, nvars)
edcpt = edrand = ednb = edrand_sym = rep(list(ed), length(datasets))
...
for (i in 1:length(datasets)) {
  ...
  for (j in 1:nvars) {
    ...
    mbrand[[j]] = forward_greedy(data, arities, vars, n, vars[j], "random", varCnt = di,
                              targetProbsAdpt = targetProbsAdpt, prior = "uniform", debug = T)
    edrand[[i]][j] = mbEditDist(mbtrue[[j]], mbrand[[j]], vars[j], vars)
    
    # mbcpt[[j]] = forward_greedy_fast(data, di, arities, n, vars[j], debug = F)
    # edcpt[[i]][j] = mbEditDist(mbtrue[[j]], mbcpt[[j]], vars[j], vars)
    # mbnb[[j]] = forward_greedy(data, arities, vars, n, vars[j], "nb", debug = F)
    # ednb[[i]][j] = mbEditDist(mbtrue[[j]], mbnb[[j]], vars[j], vars)
  }
  filename = strsplit(datasets[i], ".csv")[[1]]
  mbrand_sym = symmetry_correction(vars, mbrand, "AND")
  saveRDS(mbrand, paste0("~/Documents/Experiments/mb_discovery/child/mml_rand/", n, "/without_symmetry/", filename, ".rds"))
  saveRDS(mbrand_sym, paste0("~/Documents/Experiments/mb_discovery/child/mml_rand/", n, "/with_symmetry/", filename, ".rds"))
}

# mbrand_sym = symmetry_correction(vars, mbrand, "AND")
# mbcpt_sym = symmetry_correction(vars, mbcpt, "AND")
# mbnb_sym = symmetry_correction(vars, mbnb, "AND")
# edrand_sym = edcpt_sym = ednb_sym = rep(0, nvars)
# for (j in 1:nvars) {
#   edrand_sym[j] = mbEditDist(mbtrue[[j]], mbrand_sym[[j]], vars[j], vars)
#   edcpt_sym[j] = mbEditDist(mbtrue[[j]], mbcpt_sym[[j]], vars[j], vars)
#   ednb_sym[j] = mbEditDist(mbtrue[[j]], mbnb_sym[[j]], vars[j], vars)
# }

model = "mml_rand"
files = list.files(paste0("~/Documents/Experiments/mb_discovery/child/", model, "/", n, "/with_symmetry/"))
for (i in 1:length(files)) {
  mbrand = readRDS(paste0("~/Documents/Experiments/mb_discovery/child/", model, "/", n, "/with_symmetry/", files[i]))
  for (j in 1:length(mbrand)) {
    edrand[[i]][j] = mbEditDist(mbtrue[[j]], mbrand[[j]], vars[j], vars)
  }
}
mean(unlist(lapply(edrand, sum)))
round(1.96 * sd(unlist(lapply(edrand, sum))) / sqrt(10), 2)
```
