...
for (i in 1:length(datasets)) {
  ...
  for (j in 1:nvars) {
    ...
    ed = mb_false_finding(mbtrue[[j]], mbrand[[j]])
    cat(ed, "\n")
...
model = "mml_rand"
symtry = "with_symmetry"
files = list.files(paste0("~/Documents/Experiments/mb_discovery/child/", model, "/", n, "/", symtry, "/"))
cat(length(files), "\n")
edrand = rep(list(rep(0, nvars)), length(files))
for (i in 1:length(files)) {
  mbrand = readRDS(paste0("~/Documents/Experiments/mb_discovery/child/", model, "/", n, "/", symtry, "/", files[i]))
...
nvars = length(vars)
i = 1
lrand = lrand2 = lcpt = lnb = ltrue = list()
for (target in vars) {
...
for (target in vars) {
  ...
  crand[i, ] = classification_accuracy_mb(mbl[[i]], lrand[[i]], vars, target)
  ...
  ccpt[i, ] = classification_accuracy_mb(mbl[[i]], lcpt[[i]], vars, target)
  anb[i] = mb_false_finding(mbl[[i]], lnb[[i]])
  cnb[i, ] = classification_accuracy_mb(mbl[[i]], lnb[[i]], vars, target)
  i = i + 1
