...
This can also be justified experimentally by the following. The following code randomly samples an integer from the range $[1,28]$. And it samples in totoal 2140 times. The it marks how many times there is at least one occurence from each number. The experiment is repeated 10000 times and the result shows there is never a chance that you see 0 occurence of any one of the 28 integers.  
```{r}
cnt = 0
for (i in 1:10000) {
  k = 28
...
cnt
```
## Uniformly distribution 15 templates into 64 lines. 
```{r}
library(gmp)
p = exp(log(Stirling2(64, 15)) + log(factorial(15)) - 64 * log(15))
p
```
## Justifying the above probability experimentally. 
```{r}
cnt = 0
for (i in 1:10000) {
  k = 15
  n = 64
  x = sample(k, n, replace = T)
  l = length(unique(x))
  if (l == k) cnt = cnt + 1
}
cnt
